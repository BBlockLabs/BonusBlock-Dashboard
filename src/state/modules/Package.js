...
export default {
  ...
  getters: {
    ...
    findById: (state) => (id) => {
      return (
        state.packages.find((packageInstance) => packageInstance.id === id) ||
        null
      );
    },
    /**
     ...
     */
    filteredPackages: (state) => (filters) => {
      return state.packages.filter((packageInstance) => {
        ...
        if (
          ...
          filters.region.includes(packageInstance.server.location.country) ===
            false
        ) {
          return false;
        }
        ...
        if (
          ...
          filters.features.filter((value) =>
            packageInstance.features.includes(value)
          )?.length === 0
        ) {
          return false;
        }
        ...
        if (
          ...
          filters.storageType.filter((value) =>
            packageInstance.server.storageDriveType.includes(value)
          )?.length === 0
        ) {
          return false;
        }
  ...
  actions: {
    ...
    async loadAllPackages({ commit }) {
      ...
      const packages = packageMock.map(
        (packageData) => new Package(packageData)
      );
      packages.map(
        (packageData) =>
          (packageData.server = servers.find(
            (serverData) => serverData.id === packageData.serverId
          ))
      );
      ...
      packages.forEach((packageInstance) =>
        commit("addPackage", packageInstance)
      );
    ...
    async loadPackagesByServerId({ commit }, serverId) {
      const servers = serverMock.map((serverData) => new Server(serverData));
      const packages = packageMock
        .filter((packageData) => packageData.serverId === serverId)
