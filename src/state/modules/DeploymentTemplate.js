...
export default {
  ...
  getters: {
     ...
     */
    findById: (state) => (id) => {
      return state.deploymentTemplates.find(deploymentTemplate => deploymentTemplate.id === id) || null;
    },
  },
  mutations: {
    /**
    ...
    addDeploymentTemplate(state, deploymentTemplateInstance) {
      const existingDeploymentTemplate = state.deploymentTemplates.find(stateDeploymentTemplate => stateDeploymentTemplate.id === deploymentTemplateInstance.id);
  ...
  actions: {
    async loadAllDeploymentTemplates({commit}) {
      const deploymentTemplates = deploymentTemplateMock.map(deploymentTemplateData => new DeploymentTemplate(deploymentTemplateData));

      await sleep(500);

      deploymentTemplates.forEach(deploymentTemplateInstance => commit('addDeploymentTemplate', deploymentTemplateInstance));

      return new ActionResponse(true, deploymentTemplates);
    },
    async loadDeploymentTemplateById({commit}, deploymentTemplateId) {
      const deploymentTemplateData = deploymentTemplateMock.find(deploymentTemplateData => deploymentTemplateData.id === deploymentTemplateId) || null;

      if (deploymentTemplateData === null) {
        return new ActionResponse(false, null, ['DEPLOYMENT_TEMPLATE_NOT_FOUND']);
      }
