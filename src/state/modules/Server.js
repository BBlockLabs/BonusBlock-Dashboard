...
export default {
  ...
  mutations: {
    ...
    setServer(state, server) {
      const existingServer = state.servers.find(
        (stateServer) => stateServer.id === server.id
      );
  ...
  actions: {
    async loadAllServers({ commit }) {
      const organizations = organizationsMock.map(
        (organizationData) => new Organization(organizationData)
      );
      const servers = serverMock.map((serverData) => new Server(serverData));
      servers.map(
        (serversData) =>
          (serversData.organization = organizations.find(
            (organizationData) =>
              organizationData.id === serversData.organizationId
          ))
      );
    ...
    async loadServersByOrganization({ commit }, organizationId) {
      ...
      let servers = serverMock
        .filter((serverData) => serverData.organizationId === organizationId)
        .map((serverData) => new Server(serverData));
      ...
      servers.map(
        (serverData) =>
          (serverData.organization = organizations.find(
            (organizationData) =>
              organizationData.id === serverData.organizationId
          ))
      );
    ...
    async loadServerById({ commit }, serverId) {
      const serverData =
        serverMock.find((serverData) => serverData.id === serverId) || null;
      ...
      let organization = organizationsMock.find(
        (organizationData) => organizationData.id === server.organizationId
      );
      if (organization) {
