<template>
  <el-row v-if="loading" align="middle" justify="center" class="h-100 w-100">
  ...
  <el-container v-if="!loading" class="h-100">
    ...
    <el-container>
      <el-main>
        ...
        <highlightjs language="yaml" :code="deployment.yaml" />
      </el-main>
      <el-aside class="h-100">
        ...
        <h4>Deploying on</h4>
        <el-card v-if="deployment" shadow="never">
          <member-row />
          <template v-for="p in deployment.packages" :key="p">
            <selected-package :selected-package="p" :show-buttons="false" />
          </template>
          <el-row justify="center" class="mt-4">
            <el-button round @click="$router.push('/packages')">
              Change
            </el-button>
          </el-row>
        </el-card>
