<template>
  <div class="world-map">
    <v-chart ref="map" :option="getMapConfig" autoresize @click="handleClick" />
  </div>
</template>
...
use([CanvasRenderer, MapChart, ScatterChart, GeoComponent, TooltipComponent]);
...
export default {
  ...
  methods: {
    calculateCountryData() {
      let centroid = MapCentroids["features"].find(
        (json) => json["properties"]["COUNTRY"] === this.country
      );
      if (!centroid) {
